  ...
  useGSAP(() => {
    ...
    cards.forEach((card) => {
      ...
      .fromTo(q('.staff-bg'), 
        ...
        { scaleY: 1, opacity: 1, duration: 0.6, ease: 'expo.out' },
        '-=0.5'
      )
        ...
        >
          {STAFF.map((member, index) => (
            ...
            <div className="staff-details w-full text-left opacity-0">
              <h3 className="text-black font-bold text-[21px] leading-tight mb-1">{member.name}</h3>
              <p className="text-[#c22026] font-medium text-[14px] mb-3">{member.role}</p>
