...
type EditorEntry = { id: string; file: string; label: string; en: string; fr: string; tr: string; dz: string };
...
for (const [file, entries] of Object.entries(groups)) {
  ...
  entries.forEach((entry, index) => {
    if (!entry.en.trim() || !entry.fr.trim()) return;
    segments.push({
      id: `${file}#${index}`,
      file,
      index,
      source: entry.en,
      current: entry.fr,
      context: `${file}; ${entry.label}. Texte visible dans l’interface du site. Si c’est un CTA, rester bref. Si c’est un titre, être net. Si c’est un texte descriptif, viser une lecture naturelle.`,
    });
  });
}
...
for (let offset = 0; offset < selected.length; offset += 24) {
  ...
  const prompt = `
Tu es un éditeur francophone senior chargé de finaliser les textes d’interface du site Igloo Construction.

Réécris chaque texte dans un français naturel, professionnel et net.

Règles :
- Pas de traduction littérale de l’anglais.
- Conserve les faits, les noms propres, les chiffres et les sigles.
- Corrige tous les accents, apostrophes et espaces françaises.
- Pour les boutons, menus, intertitres et libellés : rester court et clair.
- Pour les paragraphes : écrire comme un humain, sans tournures mécaniques ou trop scolaires.
- Évite les calques comme « profil entreprise », « discussion projet », « discipline de livraison » ou « construction avec preuves ».
- Ne renvoie que le texte final en français.

Entrée JSON :
${JSON.stringify(batch)}

La sortie doit être exactement un tableau JSON :
[{"id":"...","text":"..."}]
`;
...
const artifactPath = path.join(ROOT, 'artifacts', 'translation-drafts', `editor-fr-gemini-polished-${startAt}-${startAt + results.length}.json`);
...
if (apply) {
  ...
  for (const result of results) {
    ...
    const save = await fetch(`${API}/save`, {
      ...
      body: JSON.stringify({ file: segment.file, index: segment.index, locale: 'fr', value: result.text }),
    });
    if (!save.ok) throw new Error(`Failed to save ${result.id}: ${await save.text()}`);
  }
  process.stdout.write(`Applied ${results.length} polished French editor strings.\n`);
} else {
