    ...
    <main className="bg-white text-[#111]">
      <section className="px-5 pb-16 pt-36 md:px-10 md:pb-24 md:pt-44 xl:px-16">
        <div className="mx-auto max-w-[1500px]">
          <Link to={localizedPath(locale, '/services')} className="inline-flex items-center gap-2 text-[13px] font-semibold text-black/55 transition-colors hover:text-[#c22026]"><ArrowLeft className="h-4 w-4" />Services</Link>
          <div className="mt-10 grid gap-10 lg:grid-cols-[minmax(0,1.3fr)_minmax(300px,0.7fr)] lg:items-end">
            <div>
              <p className="text-[14px] font-semibold text-[#c22026]">{page.content.eyebrow}</p>
              <h1 className="mt-6 max-w-[980px] font-nav text-[48px] font-semibold leading-[0.98] sm:text-[60px] md:text-[76px]">{page.content.heading}</h1>
            </div>
            <p className="max-w-[44ch] text-[17px] leading-[1.75] text-black/64 md:text-[19px]">{page.content.lead}</p>
          </div>
        </div>
      </section>
      ...
      <section className="bg-[#111] px-5 py-16 text-white md:px-10 md:py-24 xl:px-16">
        <div className="mx-auto grid max-w-[1500px] gap-14 lg:grid-cols-[minmax(0,1.15fr)_minmax(300px,0.85fr)] lg:gap-24">
          <div className="grid gap-7 text-[17px] leading-[1.8] text-white/70 md:text-[19px]">
      ...
      <section className="bg-[#f1f1ef] px-5 py-16 md:px-10 md:py-24 xl:px-16">
        <div className="mx-auto max-w-[1500px]">
          <div className="flex flex-col gap-5 border-b border-black/14 pb-8 md:flex-row md:items-end md:justify-between">
            <div><p className="text-[14px] font-semibold text-[#c22026]">Proof projects</p><h2 className="mt-4 text-[34px] font-semibold leading-[1.05] md:text-[48px]">Delivered work behind the service.</h2></div>
            <Link to={localizedPath(locale, '/projects')} className="inline-flex items-center gap-2 text-[14px] font-semibold hover:text-[#c22026]">All projects<ArrowRight className="h-4 w-4" /></Link>
          </div>
          <div className="grid gap-px border-b border-black/14 bg-black/14 md:grid-cols-2">
            {proofProjects.map((project) => <Link key={project.slug} to={localizedPath(locale, `/projects/${project.slug}`)} className="group bg-[#f1f1ef] p-7 transition-colors hover:bg-white md:p-9"><div className="flex items-center justify-between gap-5"><span className="text-[24px] font-semibold">{localizedProjectCardTitle(project, locale)}</span><ArrowRight className="h-5 w-5 transition-transform group-hover:translate-x-1" /></div><p className="mt-5 flex items-center gap-2 text-[14px] text-black/54"><MapPin className="h-4 w-4 text-[#c22026]" />{project.location}</p><p className="mt-5 max-w-[55ch] text-[15px] leading-[1.7] text-black/60">{project.summary}</p></Link>)}
          </div>
        </div>
      </section>

      <section className="px-5 py-16 md:px-10 md:py-24 xl:px-16"><div className="mx-auto flex max-w-[1500px] flex-col gap-7 border-t border-black/14 pt-10 md:flex-row md:items-center md:justify-between"><h2 className="max-w-[18ch] text-[34px] font-semibold leading-[1.05] md:text-[48px]">{page.content.cta}</h2><Link to={localizedPath(locale, '/contact')} className="inline-flex min-h-12 items-center gap-3 bg-black px-6 text-[14px] font-semibold text-white transition-colors hover:bg-[#c22026]">Contact Igloo Construction<ArrowRight className="h-4 w-4" /></Link></div></section>
      <Footer />
    </main>
  );
}
