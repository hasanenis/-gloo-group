    ...
    <main className="bg-white text-[#111]">
      <section className="px-5 pb-16 pt-36 md:px-10 md:pb-24 md:pt-44 xl:px-16">
        <div className="mx-auto max-w-[1500px]">
          ...
          <div className="mt-7 grid gap-10 lg:grid-cols-[minmax(0,1.25fr)_minmax(320px,0.75fr)] lg:items-end">
            <h1 className="max-w-[980px] font-nav text-[48px] font-semibold leading-[0.98] sm:text-[60px] md:text-[74px]">{page.content.heading}</h1>
            <p className="max-w-[44ch] text-[17px] leading-[1.75] text-black/64 md:text-[19px]">{page.content.lead}</p>
          </div>
          <div className="mt-14 grid gap-px border-y border-black/14 bg-black/14 md:grid-cols-2">
            {page.content.cards.map((card, index) => (
              <Link
                key={card.slug}
                to={localizedPath(locale, `/services/${card.slug}`)}
      ...
      <section className="bg-[#f1f1ef] px-5 py-16 md:px-10 md:py-24 xl:px-16">
        <div className="mx-auto grid max-w-[1500px] gap-8 md:grid-cols-[0.7fr_1.3fr] md:items-end">
          <div className="flex items-center gap-3 text-[14px] font-semibold text-[#c22026]"><MapPin className="h-4 w-4" />{brandEntity.address.addressLocality}, {brandEntity.address.addressRegion}</div>
          <div>
            <p className="max-w-[48ch] text-[21px] leading-[1.45]">{brandEntity.legalName} works from Bir Khadem, Algiers, with a portfolio of residential, commercial and infrastructure projects across Algeria.</p>
            <Link to={localizedPath(locale, '/contact')} className="mt-7 inline-flex items-center gap-2 border-b border-black pb-1 text-[14px] font-semibold transition-colors hover:border-[#c22026] hover:text-[#c22026]">
